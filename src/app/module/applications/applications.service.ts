...
const createApplication = async (
    ...
    if (existingApplication && activeStatuses.includes(existingApplication.status)) {
    throw new ApiError(
        httpStatus.CONFLICT,
...
const updateApplication = async (
    ...
    const { moveInDate, isPrivateLease, agreedRentAmount, rentalDocumentUrl, ownerFeedback, status } = payload;
    const existingApplication = await prisma.application.findUnique({
        where: { id },
    });
    ...
    if (agreedRentAmount !== undefined && user.role === Role.ADMIN) updateData.agreedRentAmount = Number(agreedRentAmount);
    if (rentalDocumentUrl !== undefined) updateData.rentalDocumentUrl = rentalDocumentUrl;
