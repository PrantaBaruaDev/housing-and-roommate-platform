...
import { IAddRoomsToFlatPayload, IRegisterPropertyInventoryPayload, IUpdateFlatPayload, IUpdateRoomPayload } from "./flats.interface";
...
const addRoomsToExistingFlat = async (
    ...
    if (!rooms || rooms.length === 0) {
        throw new ApiError(httpStatus.BAD_REQUEST, "At least one room must be provided.");
    }
...
const updateFlatDetails = async (
    ...
    if (flatName && flatName.trim().toLowerCase() !== existingFlat.flatName.toLowerCase()) {
...
const updateRoomDetails = async (
    ...
    if (roomNumber && roomNumber.trim().toLowerCase() !== existingRoom.roomNumber.toLowerCase()) {
    ...
    const updatedRoom = await prisma.rooms.update({
        ...
        data: {
            ...
            ...(rentAmount !== undefined && { rentAmount: new Prisma.Decimal(rentAmount) }),
